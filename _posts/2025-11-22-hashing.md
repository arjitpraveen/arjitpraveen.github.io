...
Let there be a hash function $h : D \rightarrow R$ that maps an input from a domain $D$ to a finite range $R$ (eg. $\vert R \vert = 2^{256}$ possible outputs for a 256 bit hash like SHA-256). If $\vert D \vert > \vert R \vert$, then at least one output in $R$ must be the image of at least two distinct inputs in $D$. IOW, there exists $x, y \subset D$ such that $h(x) = h(y)$. Moreover, we can obtain a 50% chance of finding a hash collision in $\sqrt{2^{l}} = 2^{l/2}$ trials, where $l$ is the output bit length produced b y the hash function, this is what's known as the [birthday paradox](https://en.wikipedia.org/wiki/Birthday_problem).
...
This gives us the hash function **SHA-256**. Looking up the hash-mode for SHA-256 ([here](https://hashcat.net/wiki/doku.php?id=example_hashes)) we see it's `1400`. Now we use hashcat to break it.
...
Using hashcat to crack this SHA512 hash we get,
